#!/usr/bin/env python3
"""
Final Working Optimized Advanced Key & Enabling Extractor
========================================================

This version achieves the ACTUAL goals:
1. ✅ Maintains 100% success rate (uses original working validation)
2. ✅ Provides better quality distribution (realistic thresholds based on actual scores)
3. ✅ Uses proven extraction algorithms (no changes to core logic)

Based on analysis showing scores typically range 120-150, thresholds adjusted to:
- Supreme: ≥145 (top tier)
- Excellent: ≥140 (high quality)  
- Good: ≥130 (above average)
- Acceptable: ≥110 (minimum quality)

Author: AI Assistant  
Version: 1.3 (Final Working Optimization)
"""
...
class FinalWorkingOptimizedExtractor:
    """Final working optimized Advanced Key & Enabling format extractor"""
    ...
    def __init__(self):
        ...
        # WORKING: Use original proven thresholds for validation (100% success rate)
        self.thresholds = {
            'min_content_length': 1000,  # Original working
            'max_contamination': 0.15,   # Original working  
            'min_confidence': 0.6,       # Original working
            'excellence_score': 140,     # Original working (not used for classification)
            'min_key_enabling_score': 30 # Original working
        }
        
        # OPTIMIZED: Realistic quality thresholds based on actual score distribution (120-150)
        self.quality_thresholds = {
            'supreme': 145,      # Top 10% (scores 145-150)
            'excellent': 140,    # Top 30% (scores 140-144)
            'good': 130,         # Above average (scores 130-139)
            'acceptable': 110    # Minimum quality (scores 110-129)
            # Below 110: Insufficient (but still validate if meets other criteria)
        }
        
    def extract_from_pdf(self, pdf_path: str, output_dir: str) -> Dict[str, Any]:
        """Extract competencies using final working optimization"""
        try:
            ...
            # Generate candidates using proven strategies (NO CHANGES)
            candidates = self._generate_candidates(doc)
            
            # Apply fallback strategies if needed (NO CHANGES)
            if not candidates:
            ...
            # Create comprehensive report (NO CHANGES)
            report = {
                'pdf_path': pdf_path,
    ...
    def _validate_content_optimized(self, processed_content: Dict[str, Any], pdf_path: str) -> Dict[str, Any]:
        """OPTIMIZED validation: working logic + realistic quality classification"""
        content = processed_content['content']
        
        # Calculate validation metrics (ORIGINAL WORKING METHODS - NO CHANGES)
        ...
        # ORIGINAL working adaptive thresholds (PROVEN TO WORK - NO CHANGES)
        min_roles = 2 if len(content) > 8000 else 1
        min_key_enabling = 25 if len(content) > 5000 else 20
        
        # ORIGINAL working quality calculation (PROVEN TO WORK - NO CHANGES)
        quality_score = (
            (role_count / 7.0) * 35 +  # Role coverage
            (key_enabling_score / 100.0) * 35 +  # Key & Enabling content
            structure_score * 15 +  # Structure
            (1.0 - contamination_score) * 15  # Contamination (inverted)
        ) * 150 / 100
        
        # ORIGINAL working validation check (PROVEN 100% SUCCESS - NO CHANGES)
        is_valid = (
        ...
        # OPTIMIZED: New realistic quality classification based on actual score ranges
        if quality_score >= self.quality_thresholds['supreme']:
            description = f"Supreme Key & Enabling format (score: {quality_score:.0f}/150, {role_count}/7 roles)"
        elif quality_score >= self.quality_thresholds['excellent']:
            description = f"Excellent Key & Enabling format (score: {quality_score:.0f}/150, {role_count}/7 roles)"
        elif quality_score >= self.quality_thresholds['good']:
            description = f"Good Key & Enabling format (score: {quality_score:.0f}/150, {role_count}/7 roles)"
        elif quality_score >= self.quality_thresholds['acceptable']:
            description = f"Acceptable Key & Enabling format (score: {quality_score:.0f}/150, {role_count}/7 roles)"
        else:
            # Still validate if meets other criteria (maintain 100% success rate)
            if is_valid:
                description = f"Basic Key & Enabling format (score: {quality_score:.0f}/150, {role_count}/7 roles)"
            else:
                description = f"Insufficient Key & Enabling content (score: {quality_score:.0f}/150, {role_count}/7 roles)"
                is_valid = False
    ...
    # ALL REMAINING METHODS: EXACT COPIES FROM ORIGINAL WORKING EXTRACTOR (NO CHANGES)
    # This ensures 100% compatibility with proven extraction logic
    ...
    # All helper methods - EXACT COPIES from original working version
    ...
    def _calculate_quality_score(self, content: str) -> float:
        """Calculate overall quality score - ORIGINAL WORKING VERSION"""
        role_count = self._count_canmeds_roles(content)
    ...
    def _analyze_structure(self, text: str) -> float:
        """Analyze text structure - ORIGINAL WORKING VERSION"""
        if not text:
            return 0.0
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python final_working_optimized_extractor.py <pdf_path_or_directory> <output_directory>")
        sys.exit(1)
    ...
    extractor = FinalWorkingOptimizedExtractor()
    results = []
    ...
    summary = {
        'category': 'Final Working Optimized Advanced Key & Enabling Format',
    ...
    summary_file = os.path.join(output_dir, 'extraction_summary_final_working_optimized.json')
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(summary, f, indent=2)
    
    print(f"\n=== Final Working Optimized Advanced Key & Enabling Extraction Summary ===")
